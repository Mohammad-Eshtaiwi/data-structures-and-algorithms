...
const createServer = () => {
  ...
  app.get("/aboutme", (req, res) => {
    res.send("cats lover and programmer");
  });
  app.get("/favoritefoods", (req, res) => {
    res.send(["spigiti", "shrimps"]);
  });
  app.get("*", (req, res) => {
...
const count = (target, input) => {
  ...
  return input.reduce((acc, val) => {
    val.forEach((item) => (item === target ? ++acc : 0));
    return acc;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map((arr) => {
    let finalArr = arr.filter(
      (item) => item % 5 === 0 && typeof item === "number"
    );
    return finalArr.map((item) => Math.pow(2, item));
  });
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce(
    (acc, { name }) => (acc.length >= name.length ? name : acc),
    data[0].name
  );
};
