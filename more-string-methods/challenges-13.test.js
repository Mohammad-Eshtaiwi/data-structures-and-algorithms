...
const standardizePhoneNumbers = (arr) => {
  ...
  return arr.map((num) => {
    num = num.split("");
    num = num.filter((item) => item.match(/[0-9]/));
    return num.join("");
  });
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  str = str.split("");
  str = str.filter((char, index) => index % 2 !== 0);
  console.log(str);
  return str.join("");
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map((item) => item.filter((ele) => !ele.includes("Brook")));
};
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach((day) => {
    let arr1 = [];
    arr.forEach((event) => {
      if (event.includes(day)) arr1.push(event);
    });
    result.push(arr1);
  });
  return result;
...
Write a function named characterByIndex that takes in an array of strings and returns an array containing the first character of the first string,
the second character of the second string, etc.
