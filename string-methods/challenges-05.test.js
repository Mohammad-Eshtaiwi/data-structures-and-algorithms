...
Write a function named templateWithJQuery that uses jQuery to get the html template from the DOM, copy the contents,
fill it with the Star Wars People, and append it to the DOM.
------------------------------------------------------------------------------------------------ */
let starWarsPeople = [
  {
    name: "Luke Skywalker",
...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach((person) => {
    let section = `<section id="template">
    <h2>${person.name}</h2>
    <h3>${person.height}</h3>
...
const howMuchPencil = (str) => {
  ...
  for (let letter = 0; letter < str.length; letter++) {
    result.push(str.substring(letter, str.length));
  }
  result.push("");
...
const wordsToCharList = (arr) => {
  ...
  for (let letter = 0; letter < arr.length; letter++) {
    result.push(arr.substring(letter, letter + 1));
  }
  return result;
};
